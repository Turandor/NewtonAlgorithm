        ...
        public void CalculateNewtonAlgorithm(double[] xStart)
        {
            ...
            while (greaterLoop)
            {
                ...
                if (i != 0)
                {
                    ...
                    if (criterionGradient[i - 1] <= epsilon)                              // Kryterium <gradient(x_i),gradient(x_i)> <= epsilon
                    {
                        criterionText = "Iloczyn skalarny gradientu";
                        return;
                    }
                    else if (criterionX[i - 1] <= epsilon)  // Kryterium ||x_i - x_i-1|| <= epsilon
                    {
                        criterionText = "Długość pomiędzy x_n i x_n-1";
                        return;
                    }
                    else if (criterionFx[i - 1] <= epsilon)                      // Kryterium |f(x_i) - f(x_i-1)| <= epsilon
                    {
